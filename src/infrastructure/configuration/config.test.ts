...
import { loadConfig, loadStoredConfig, setConfig, setModelSelection } from "./config.js";
...
test("config migrates legacy Gemini files and persists provider selections", async () => {
  ...
  try {
    ...
    assert.deepEqual(await loadConfig(), { provider: "gemini", model: "gemini-legacy" });
    ...
    assert.deepEqual(await loadConfig(), { provider: "groq", model: "custom/groq" });
  } finally {
