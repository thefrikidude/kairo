...
test("context compacts long history into a durable checkpoint", async () => {
  const dir = await mkdtemp(join(tmpdir(), "kairo-context-")); const store = await SessionStore.open(join(dir, "sessions.sqlite")); const session = store.create("/workspace"); const task = store.startTask(session.id, "understand the project");
