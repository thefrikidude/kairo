...
export class ContextManager {
  ...
  prepare(sessionId: string, task: Task): Message[] {
    ...
    return [{ role: "user", content: `Context checkpoint from earlier work:\n${checkpoint.summary}`, createdAt: checkpoint.createdAt }, ...recent];
  }
  compact(sessionId: string, task: Task): string {
    ...
    const recent = this.store.recentMessages(sessionId, 12).map((message) => `${message.role}${message.toolName ? `:${message.toolName}` : ""}: ${excerpt(message.content, 420)}`).join("\n");
    const summary = [
      `Task: ${task.prompt}`,
      `State: ${task.status}`,
      ...
      "Recent durable evidence:", recent
    ].filter(Boolean).join("\n");
